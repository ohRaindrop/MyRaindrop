...
<div class="centro">
<br>
Benvenuto nella sezione dedicata agli acquisti.<br>
Per effettuare gli acquisti, clicca sulla checkbox. <br>
<br>
Ricordati che se lasci questa pagina, dovrai effettuare di nuovo il login.
<br><br>
 <?php

include   "database.inc";

$select= "SELECT foto,nome,prezzo,id_p FROM Prodotti;";
...
echo("<FORM ACTION='stampa_fattura.php' METHOD='post'>");
while($riga)
{
  echo("<p>");
  echo("<img src='$riga[foto]' width=200px height=200px>");
  echo("  $riga[nome] ");
  echo(" al prezzo di: $riga[prezzo] €");
  echo(" <input type='checkbox' name='id_prodotti[]' value='$riga[id_p]'>");
  echo(" <input type='text' name='q[]' value=0>");
  echo("</p>");
  $riga=mysql_fetch_array($ris, MYSQL_ASSOC);
}

echo("<br><br>");
echo("<input type='hidden' name='id_c[]' value='$_POST[id_c]'> <br><br>");

$rand=rand(0,150000);
echo("Per migliorare la sicurezza, inserire il numero che vede qui: $rand
<input type='text' name='id_ordine[]'> <br><br>");

echo("<input type='submit' value='Concludi acquisti'>");
echo("</FORM>");
