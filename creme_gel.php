...
$select= "SELECT foto, nome, prezzo, descrizione, denominazione FROM (Categoria NATURAL JOIN Prodotti) INNER JOIN Fornitore ON prodotti.id_f=fornitore.id_f  WHERE nominata='Creme corpo' OR nominata='Creme viso' OR nominata='Creme capelli' OR nominata='Gel';";
...
if(!$riga)
{
  echo("Nessuna marca o prodotti trovati. <br><br> Errore: ".mysql_error());
  exit();
}

echo("<center><h1><b>Lista di tutte le creme (viso, corpo, capelli) e dei gel :</b></h1></center>");
while($riga)
{
  ...
  echo("Fornito da:  $riga[denominazione].");
  echo("<br><br>");
