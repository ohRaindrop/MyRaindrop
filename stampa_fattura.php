...
foreach ($_POST["id_prodotti"] as $id_p)
        foreach ($_POST["q"] as $qta)
                 if ($qta > 0 )
                    foreach ($_POST["id_ordine"] as $id_ordine)
                            foreach ($_POST["id_c"] as $id_c)
                            $insert="INSERT INTO Ordina (id_ordine,id_p,id_c,data,qta) VALUES ('$id_ordine','$id_p','$id_c', curdate(), $qta)";
...
$select="Select foto,nome,prezzo from (Ordina NATURAL JOIN Prodotti) WHERE id_ordine='$id_ordine';";
...
if(!$riga)
{
  echo("Nessun prodotto trovato. <br> Errore: ".mysql_error());
  exit();
}

echo("<center><h1>Lista di tutti i prodotti acquistati:</h1></center>");
$tot=0;
$parz=0;
...
echo("<br><br>");
echo("Il totale risulta= $tot €<br><br>");
...
<a href='conferma_fattura.htm'> Conferma gli acquisti. </a>
