...
$select = "SELECT * FROM Cliente WHERE mail='$_POST[mail]'";
...
if (!$riga or $riga['password']!=$_POST['password'])
{
echo ("Non c'è corrispondenza tra la mail e la password, oppure l'utente non è presente nel database. Riprovare.");
exit();
}
else
{
  echo("Accesso consentito. <br>") ;
  $select2="SELECT id_c FROM Cliente WHERE mail='$_POST[mail]' and password='$_POST[password]'";
  $ris=mysql_query($select2);
  $riga2=mysql_fetch_array($ris);
  echo("<FORM ACTION=fattura.php METHOD=post>");
  echo("<input type='hidden' name='id_c' value='$riga2[id_c]' <br><br>");
  echo("<input type=submit value='Procedi con gli acquisti'");
}
mysql_close($con);
?>
